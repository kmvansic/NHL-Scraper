...
class ESPNSportsObj:
	...
	def startDay(self):
		...
		gamelist = tree.xpath('//*[@class="shsScoreboardCol"]')
		count = len(gamelist)
		...
		for game in range (1,count):
			...
			if (tree.xpath('//*[@id="shsScoreboard"]/div[2]/div[' + str(game) + ']/table//*[@class="shsTotD"][4]/text()')[0]=="Tot"):
				homeScore = tree.xpath('//*[@id="shsScoreboard"]/div[2]/div[' + str(game) + ']/table//*[@class="shsTotD"][4]/text()')[2]	
				awayScore = tree.xpath('//*[@id="shsScoreboard"]/div[2]/div[' + str(game) + ']/table//*[@class="shsTotD"][4]/text()')[1]
			else:
				homeScore=0
				awayScore=0
			proposedTime = tree.xpath('//*[@id="shsScoreboard"]/div[2]/div[' + str(game) + ']/table//*[@class="shsTimezone shsCTZone"]/text()')
			if proposedTime==[]:
				proposedTime=""
			else:
...
def get_ESPN_url(homeTeam):
    NYT_to_ESPN_dict={'Anaheim':'Ducks',
                      'Arizona':'Coyotes',
                      'Boston':'Bruins',
                      'Buffalo':'Sabres',
                      'Calgary':'Flames',
    ...
    bidict_NYT_to_ESPN = bidict(NYT_to_ESPN_dict)
    bidictHomeTeam = bidict_NYT_to_ESPN[homeTeam]	
    ...
    try:
        page = requests.get('http://espn.go.com/nhl/scoreboard?date='+getDateStr(),headers=headers)
    except:
        return;
    tree = html.fromstring(page.content);
	#Retrieve ids for each game on the current day.
	#An id is defined by ESPN as a unique numerical identifier for a given game
	#id is used to more directly access elements of the XML using XPATH
    idList = tree.xpath('//*[@id="fittPageContainer"]/div[3]/div/div/div[1]/div/section/div/section/@id')
	#Add each game to the games list with initialized data
    for id in idList:
		#find the urls of each game in the scoreboard. dissselect the recap headlines which are found similarly
